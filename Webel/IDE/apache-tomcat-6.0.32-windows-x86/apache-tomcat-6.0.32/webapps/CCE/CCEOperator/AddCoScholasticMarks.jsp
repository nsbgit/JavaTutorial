...
<title>CCE | <s:if test="(status.equals('insert'))">Add</s:if><s:else>View</s:else> Co-Scholastic Marks</title>
</head>
<body>
		...
		<s:div cssClass="container1">
			<s:div cssClass="wrapper">
				...
				<s:div cssClass="rightside">
						<h1><s:if test="(status.equals('insert'))">Add</s:if><s:else>View</s:else>  Co-Scholastic Marks</h1>
						<div class="form">
							<table >						
								<tr>
	...
	function redirect(type){
		...
		var subArea=document.getElementById('subArea').value;			
			if( academicYear !='' && standard !='' && area !='' && subArea !='' ){
				if(type=='insert')		
				{	
					document.getElementById('a').href="marksEntry.html?academicYear="+academicYear+"&standard="+standard+"&area="+area+"&subArea="+subArea+"&keepThis=true&TB_iframe=true&height=550&width=950";
				}
				else
				{						
					document.getElementById('a').href="marksEdit.html?academicYear="+academicYear+"&standard="+standard+"&area="+area+"&subArea="+subArea+"&keepThis=true&TB_iframe=true&height=550&width=950";
				}
				return true;
			}
			else
